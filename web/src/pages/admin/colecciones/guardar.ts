...
export const POST: APIRoute = async ({ request, locals, redirect }) => {
  ...
  if (!nombre) return redirect('/admin/colecciones');
  ...
  if (file && file.size > 0) {
    ...
    imagen = '/uploads/colecciones/' + filename;
  }
  db.prepare('INSERT INTO colecciones (nombre, slug, descripcion, imagen) VALUES (?,?,?,?)').run(nombre, slugify(nombre), descripcion, imagen);
